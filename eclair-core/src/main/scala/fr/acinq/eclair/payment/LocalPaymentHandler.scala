...
import fr.acinq.eclair.payment.PaymentLifecycle.{CheckPayment, CheckPaymentDetailed, ReceivePayment}
import fr.acinq.eclair.wire._
import fr.acinq.eclair.{Globals, NodeParams, randomBytes}
import fr.acinq.eclair.api.CheckPaymentDetailedResponse
...
class LocalPaymentHandler(nodeParams: NodeParams) extends Actor with ActorLogging {
  ...
  def run(hash2preimage: Map[BinaryData, PendingPaymentRequest]): Receive = {
    ...
    import fr.acinq.eclair.api.CheckPaymentDetailedResponse
    case CheckPaymentDetailed(paymentHash) =>
      sender ! CheckPaymentDetailedResponse(nodeParams.paymentsDb.findByPaymentHash(paymentHash))
